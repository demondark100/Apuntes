...
function CachWebJs() {
  return (  
    <>
      <Resumenes contenido={[{
        mensaje: `Con esto haremos que la web pueda guardarse y abrirse de forma offline.`,
        lenguage: "JavaScropt",
        codigo: `let nombre = "version 1";

self.addEventListener("install",e=>{
  caches.open(nombre).then(cache=>{
    cache.add("miWrb.html").then(res=>{
  ...
  caches.open(nombre).then(cache=>{
    cache.add("Cachear sitio web y mostrarlo offline.html").then(res=>{
      ...
      console.log(e);
        ...
        <JavaScropt codigo={`self.addEventListener("activate",e=>{
  caches.keys().then(key=>{
    return Promise.all(
      key.map(cache=>{
        if (cache != nombre) {
